...
export function AlbumPasswordForm({ albumSlug, albumConfig }: AlbumPasswordFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/auth/login?albumSlug=${encodeURIComponent(albumSlug)}`, {
        method: 'POST',
        headers: {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl font-bold">{albumConfig.title}</CardTitle>
          <CardDescription>
